...
object MultiBlock2D {
  ...
  def iniGeometry(lattice:MultiLattice2D, units:UnitsConverter) {
  ...
  def main( args: Array[String] ) : Unit = {
    ...
    val lattice = new MultiLattice2D( units.nX, units.nY,new BGKdynamics(D2Q9,units.omega), 2, 3 )
    val analytic = new Poiseuille2D(units,0)
...
//     val disp = Image( lattice.map( C => Math.sqrt(Arrays.normSqr(C.u)) ) ).display
    ...
    for (iT <- 0 until maxT) {
      ...
      if (iT % logT == 0) println("iT = " + iT + ", L-2 error = "+MultiAverages.velocityL2Error(lattice, analytic.velocity))
//       if (iT % logT == 0) {
//         actor {
...
//     val disp = Image( lattice.map( C => Math.sqrt(Arrays.normSqr(C.u)) ) ).display
    ...
    for (iT <- 0 until maxT) {
      lattice.collideAndStream
      if (iT % logT == 0) println("iT = " + iT + ", L-2 error = "+Averages.velocityL2Error(lattice, analytic.velocity))
//       if (iT % logT == 0) {
//         actor {
//             disp.update( lattice.map( C => Math.sqrt(Arrays.normSqr(C.u)) ) )
//         }

...
class Image( private val matrix: Array[Array[Double]] ) {
  ...
  lazy val paintScale =  new GrayPaintScale( minMax._1, minMax._2 )
  ...
  lazy val legend = {
    ...
    scaleAxis setUpperBound minMax._2
    scaleAxis.setAxisLinePaint(Color.white)
    scaleAxis.setTickMarkPaint(Color.white)
    scaleAxis.setTickLabelFont(new Font("Dialog", Font.PLAIN, 12))
    val  legend = new PaintScaleLegend(paintScale, scaleAxis)
...
object Image {
  ...
  def apply(matrix: Array[Array[Double]] ) = new Image(matrix)
